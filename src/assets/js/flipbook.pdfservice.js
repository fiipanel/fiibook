...
FLIPBOOK.PdfService = function (pdfDocument, main, options) {
  ...
  main.on('turnpagecomplete', function(e){
    self.rightIndex = e.rightIndex
    self.clearCanvasBuffer()
  })

  main.on('lightboxclose', function () {
    self.clearCanvasBuffer(true)
  })

  this.getCanvas = function (size) {
  ...
  this.clearCanvasBuffer = function (lightboxClose) {

    var self = this
    var unloadedPages = []
    var d = this.options.isMobile ? 6 : 50
    if(lightboxClose) d = 2
    ...
    if (unloadedPages.length > 0)
      this.main.trigger('pageUnloaded', { unloadedPages: unloadedPages })
  ...
  this.findInPageTextContentAvailable = function (page, index) {
    ...
    while (str.toUpperCase().indexOf(self.searchingString.toUpperCase(), startIndex + 1) > -1) {
      startIndex = str.toUpperCase().indexOf(self.searchingString.toUpperCase(), startIndex + 1)
    ...
    if (callback)
      callback.call(this, matches, page.htmlContent, page._pageIndex, str.substring(0, 100) + '...')
  ...
  this.renderPage = function (page, size, callback) {
    ...
    if (page.canvas[size] && page.canvas[size].rendering) {
  ...
  this.loadPageTextLayer = function (pageIndex, bookPageIndex, callback) {
    ...
    if ((self.options.annotationLayer && !page.annotationsLoaded) || (self.options.pdfTextLayer && !page.textContentLoaded)) {
      setTimeout(function () {
        self.loadPageTextLayer(pageIndex, bookPageIndex, callback)
      }, 100)
  ...
  this.Linkify = function (inputText) {
    ...
    var replacedText = inputText.replace(replacePattern1, '<a href="$1" target="_blank" class="flipbook-page-auto-link">$1</a>');

    //URLs starting with www. (without // before it, or it'd re-link the ones done above)
    var replacePattern2 = /(^|[^\/])(www\.[\S]+(\b|$))/gim;
    var replacedText = replacedText.replace(replacePattern2, '$1<a href="http://$2" target="_blank" class="flipbook-page-auto-link">$2</a>');

    //Change email addresses to mailto:: links
    var replacePattern3 = /(([a-zA-Z0-9_\-\.]+)@[a-zA-Z_]+?(?:\.[a-zA-Z]{2,6}))+/gim;
    var replacedText = replacedText.replace(replacePattern3, '<a href="mailto:$1" class="flipbook-page-auto-link">$1</a>');
  ...
  this.onBookPageRendered = function (page, canvas, index, size) {
    ...
    this.main.trigger('pageLoaded', { index: index, size: size, canvas: canvas, images: page.image[size] })
  ...
  this.onPdfPageImageReady = function(page, size, callback){
    ...
    if (callback)
      callback.call(self, { canvas: c, image: page.image, /*lCanvas: lCanvas, rCanvas: rCanvas, */size: size, pdfPageIndex: pdfPageIndex, htmlContent: page.htmlContent });
    callback = null
  ...
  this.onPdfPageRendered = function (page, size, callback) {
    ...
    if(page.image[size]){
      ...
      // create duplicate image if needed
      // release canvas
      ...
      function blobReady(blob) {
        ...
        img.onload = function() {
          ...
          if(!revoked && (!imgClone || imgClone.loaded)){
            ...
            c.available = true
            c.rendered = false
            c.creatingBlob = false
            self.onPdfPageImageReady(page, size, callback)
          }
        ...
        if(options.doublePage && pdfPageIndex > 0 && pdfPageIndex < options.pages.length / 2 && !self.webgl){

          imgClone = document.createElement('img');
          imgClone.onload = function(){
            imgClone.loaded = true
            
            if(!revoked && img.loaded){
              URL.revokeObjectURL(url);
              revoked = true
              //unload canvas
              c.width = c.height = 1
          ...
          page.image[size].clone = imgClone
      ...
      if(typeof OffscreenCanvas != 'undefined'){
        c.convertToBlob({type: 'image/webp'}).then(blobReady)

      }else{
        c.toBlob(blobReady, 'image/webp');
